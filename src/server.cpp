...
void Server::start() {
    ...
    while (true) {
        ...
        if (FD_ISSET(dest_socket_, &fds)) {
            ...
            if (client_fd >= 0) {
                //  Creates a ClientHandler, passing it:
                //      A filedescriptor for it's socket
                //      A lambda function that captures the server object and calls Server::broadcastMessage to use in callback function.
